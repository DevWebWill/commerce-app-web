        ...
        <>
            <div className='relative h-28 w-32 rounded-xl overflow-hidden group cursor-pointer transition-all duration-300 lg:w-28 md:w-24 sm:w-16 lg:h-24 md:h-20 sm:h-14'>
                ...
                <video
                    ...
                    muted={true}
                    playsInline={true}
                    /* `#t=${t}` Truco para que el video se reprodusca a partir de un tiempo determinado */
                    src={clip+`#t=${t}`}
                    alt='Video'
                    className="absolute top-0 left-0 right-0 flex h-full w-full object-cover opacity-1 z-50 rounded-xl"
                />
            </div>
        </>
    )
